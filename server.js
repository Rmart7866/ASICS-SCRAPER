// server.js - Simple Auto-Scraper (No Shopify)
...
class SimpleASICSScraper {
    ...
    async init() {
        console.log('🚀 Initializing Simple ASICS Auto-Scraper...');
    ...
    async scrapeAllPages() {
        ...
        console.log(`🚀 Starting scrape of ${this.pageList.length} pages`);
    ...
    getDashboardHTML() {
        ...
        <html>
        <head>
            <title>ASICS Simple Auto-Scraper</title>
            <meta charset="utf-8">
        ...
        <body>
            <div class="container">
                <div class="header">
                    ...
                    <p>Simple inventory monitoring (No Shopify - Just scraping & storage)</p>
                </div>
                
                <div class="cards">
                    <div class="card">
                        <h3>🎛️ Controls</h3>
                        <button class="btn" id="scrapeBtn" onclick="scrapeNow()">Scrape All Pages Now</button>
                ...
                async function refreshStatus() {
                    try {
                        ...
                        if (status.isScrapingNow) {
                            ...
                            btn.textContent = 'Scrape All Pages Now';
                            btn.disabled = false;
                        }
                ...
                async function scrapeNow() {
                    ...
                    try {
                        ...
                        if (result.success) {
                            ...
                            setTimeout(refreshStatus, 10000); // Check again in 10s
                        }
                    ...
                    try {
                        ...
                        pageListDiv.innerHTML = data.pages.map(url => \`
                            <div class="page-item">
                                ...
                                <button class="btn btn-danger" onclick="removePage('\${url}')" style="padding: 6px 12px; font-size: 12px;">Remove</button>
                            </div>
                        \`).join('');
                ...
                async function loadInventoryStats() {
                    try {
                        ...
                        if (data.inventory.length === 0) {
                            statsDiv.innerHTML = '<p style="grid-column: 1/-1; text-align: center; color: #666;">No inventory data yet</p>';
                            return;
                        }
                        
                        const totalProducts = data.inventory.length;
                        const totalQuantity = data.inventory.reduce((sum, item) => sum + (item.total_quantity || 0), 0);
    ...
    start() {
        ...
        this.app.listen(port, '0.0.0.0', () => {
            console.log(`🚀 Simple ASICS Auto-Scraper running on port ${port}`);
...
// Start the scraper
const scraper = new SimpleASICSScraper();
scraper.start();
